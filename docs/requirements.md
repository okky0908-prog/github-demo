...
ボード・リスト・カードの3階層でタスクを管理する。ボードは1つのみ。カードは作成・編集・削除・ステータス（所属リスト）変更・並び替え（ドラッグ&ドロップ、優先度順/期限順ソート）に対応する。リストは初期状態（未着手／進行中／完了）を利用する読み取り専用で、リスト自体の作成・名前変更・削除は現時点では未実装。
