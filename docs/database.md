...
- カード削除（`DELETE /api/cards/{cardId}`）は実装済み。カードに子レコードはないためカスケードの論点はなく、削除後は同一リスト内の残りカードの`position`を振り直す
- リスト削除は現時点で未実装（[機能要件](./features.md)参照）。将来リスト削除APIを実装する際は、内包するカードも削除する仕様のため、DB制約（`ON DELETE CASCADE`への変更）またはアプリケーション層（サービスクラスでの明示的な子レコード削除）のいずれかでカスケード削除を保証する必要がある
