...
## 3. アーキテクチャ設計（最小構成・Dockerなし）

個人利用・認証なしのスクール課題であり、他者が使う想定もないため、一般的なWebアプリ構成（nginx + アプリ + DBの3コンテナ構成）はオーバースペックと判断し、**Dockerを使わずEC2インスタンス上でSpring Bootの1プロセスのみを稼働させる構成**に変更する。
...
[ブラウザ] → (80番ポート) → [EC2インスタンス]
                                 └─ Spring Boot（組み込みTomcat、1プロセス）
                                       ...
                                       └─ PostgreSQLへlocalhost経由で接続（同一インスタンス内、外部非公開）
```

- **Dockerは使用しない**。Java・PostgreSQLはEC2インスタンス上に直接インストールする（`user_data.sh`で自動セットアップ）。個人利用・単一プロセスで完結する規模のため、コンテナ・オーケストレーションの複雑さを持ち込まない
...
- **PostgreSQLもネイティブインストール**し、バックエンドはlocalhost経由で接続する。外部には一切公開しない（ローカル開発環境ではDocker ComposeでPostgreSQLのみコンテナ起動しているが、本番のEC2側ではコンテナを使わない点が異なる）
- **メモリ対策**：`t2.micro`/`t3.micro`は1GB RAMしかなく、アプリのビルドや実行でメモリ不足になりやすいため、`user_data.sh`内でスワップファイル（1GB程度）を作成しておく
...
### この構成にするために必要なアプリ側の変更（実装時、別Issueで対応）
...
└── user_data.sh         # EC2起動時に自動実行するスクリプト
```

`user_data.sh`が行う処理（想定）：

1. スワップファイルの作成（メモリ対策）
2. Java・PostgreSQLのインストール、PostgreSQLの初期設定（DB・ユーザー作成）
3. アプリのリポジトリを取得し、フロントエンド・バックエンドをビルド
4. ビルドしたアプリをsystemdサービスとして登録し、自動起動・再起動時の復旧に対応させる
