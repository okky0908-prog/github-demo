...
public class CardController {
  ...
  public CardResponse createCard(
      @PathVariable UUID listId, @Valid @RequestBody CardCreateRequest request) {
    TaskList list =
        taskListRepository
            .findById(listId)
            .orElseThrow(
                () ->
                    new ResponseStatusException(HttpStatus.NOT_FOUND, "list not found: " + listId));
  ...
  public CardResponse updateCard(
      @PathVariable UUID cardId, @Valid @RequestBody CardCreateRequest request) {
    Card card =
        cardRepository
            .findById(cardId)
            .orElseThrow(
                () ->
                    new ResponseStatusException(HttpStatus.NOT_FOUND, "card not found: " + cardId));
  ...
  public CardResponse moveCard(
      @PathVariable UUID cardId, @Valid @RequestBody CardMoveRequest request) {
    Card card =
        cardRepository
            .findById(cardId)
            .orElseThrow(
            ...
            .orElseThrow(
                () ->
                    new ResponseStatusException(
                        HttpStatus.NOT_FOUND, "list not found: " + request.listId()));
