...
- ボード・リスト・カードの3階層によるタスク管理（ボードは1つのみ、リストは現時点で読み取り専用）
- カードの作成／編集／削除、ステータス（所属リスト）の直接指定・変更
- カードのドラッグ&ドロップ（リスト間移動、リスト内並び替え）、優先度順/期限順でのリストごとのソート表示
...
- [x] カード（タスク）読み取りAPI
- [x] フロントエンド環境構築、ボード画面実装
- [x] カード（タスク）の作成・編集API・画面
- [x] ドラッグ&ドロップ（リスト間移動・並び替え）
- [x] カードの削除API・画面（物理削除）
